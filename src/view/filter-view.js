...
function creatFilterTemplate(points) {
  const dateNow = Date.now();

  const checkDisabled = points.map((point) => {
    const currentDate = new Date(point.dateTo);
    const disableDate = dateNow > currentDate.getTime() ? 'disabled' : 'included';
    return disableDate;
  });

  const disabled = checkDisabled.find((disabledPoint) => disabledPoint === 'included') ? '' : 'disabled' ;
